...
def _linux_cpu_times() -> tuple[int, int] | None:
    ...
    if not os.path.isfile(path):
        return None
    try:
        first = open(path, "r", encoding="utf-8").readline().split()
    except OSError:
        return None
...
def _cpu_utilization(previous: tuple[int, int] | None, current: tuple[int, int] | None) -> float | None:
    ...
    return min(100.0, max(0.0, busy * 100.0 / total_delta))
...
def summarize_telemetry(samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for sample in samples:
        cpu = sample.get("cpu_util_percent")
        ...
        if isinstance(used, (int, float)):
            memory_used.append(float(used))
        for gpu in sample.get("gpus", []):
            if not isinstance(gpu, dict):
                continue
            key = str(gpu.get("uuid") or gpu.get("index"))
            bucket = gpu_by_key.setdefault(key, {
                "gpu_util_percent": [], "memory_util_percent": [], "memory_used_mib": [],
    ...
    gpus = {key: {metric: _numeric_summary(values) for metric, values in metrics.items()} for key, metrics in gpu_by_key.items()}
    return {
        "sample_count": len(samples),
        "cpu_util_percent": _numeric_summary(cpu_values),
        "memory_used_bytes": _numeric_summary(memory_used),
        "gpus": gpus,
    }
...
class TelemetrySampler:
    ...
    _last_cpu_times: tuple[int, int] | None = field(default=None, init=False, repr=False)

    def _capture(self) -> None:
        current = _linux_cpu_times()
        snapshot = telemetry_snapshot()
        snapshot["cpu_util_percent"] = _cpu_utilization(self._last_cpu_times, current)
    ...
    def start(self) -> None:
        ...
        self._last_cpu_times = _linux_cpu_times()
        self._capture()
